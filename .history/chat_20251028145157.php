                                                    ...
                                                    <div class="flex items-start gap-3" :class="{'justify-end' : selectedUser.userId === message.fromUserId}">
                                                        <div class="flex-none" :class="{'order-2' : selectedUser.userId === message.fromUserId}">
                                                            <template x-if="selectedUser.userId === message.fromUserId">
                                                                <img :src="`assets/images/${loginUser.path}`" class="h-10 w-10 rounded-full object-cover">
                                                            </template>
                                                            <template x-if="selectedUser.userId !== message.fromUserId">
                                                                <img :src="`assets/images/${selectedUser.path}`" class="h-10 w-10 rounded-full object-cover">
                                                            </template>
                                                        </div>
                                                        <div class="space-y-2">
            ...
            document.addEventListener('alpine:init', () => {
                ...
                Alpine.store('chat', {
                    ...
                    loginUser: {
                        id: 0,
                        name: 'Alon Smith',
                        path: 'assets/images/profile-34.jpeg',
                    ...
                    sendMessage() {
                        if (this.textMessage.trim() && this.selectedUser && socket && socket.readyState === WebSocket.OPEN) {
                            ...
                            this.messages.push({
                                fromUserId: Alpine.store('user').id, // current user id
                            ...
                            console.error('Cannot send message: WebSocket not connected or no user selected');
                        }
                    },
    ...
    async function loadUserData() {
        try {
            ...
            if (data.success) {
                ...
                // window.location.href = 'auth-cover-login.php';
            }
        } catch (err) {
            console.error('Failed to load user data:', err);
            // Optionally, redirect to login or show error
