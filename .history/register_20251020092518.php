...
if (!$first || !$last || !$email || !$pass || !filter_var($email, FILTER_VALIDATE_EMAIL) || !in_array($role, ['administrator','user'])) {
    session_start();
...
try {
    ...
    $ins = $pdo->prepare('INSERT INTO users (first_name, last_name, email, password_hash, role, created_at) VALUES (?, ?, ?, ?, ?, NOW())');
    $ins->execute([$first, $last, $email, $password_hash, $role]);
