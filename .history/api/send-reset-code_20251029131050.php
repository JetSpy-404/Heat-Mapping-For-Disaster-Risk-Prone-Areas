...
try {
    ...
    // Use PHP's built-in mail function (for testing, we'll simulate success)
    // In production, configure SMTP settings in php.ini or use a proper mail library
    $mailSent = true; // Simulate success for testing

    // Uncomment the line below for actual email sending (requires SMTP configuration)
    $mailSent = mail($email, $subject, $message, implode("\r\n", $headers));

    if ($mailSent) {
        echo json_encode(['success' => true]);
    } else {
        error_log('[send-reset-code] Mail function failed');
