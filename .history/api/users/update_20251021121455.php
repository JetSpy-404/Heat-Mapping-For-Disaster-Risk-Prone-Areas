...
try {
    // Authorization: only admins may change role
    $currentRole = $_SESSION['user_role'] ?? 'user';
    if ($usertype !== '' && $usertype !== null && $currentRole !== 'admin') {
        // non-admin attempting to change role -> forbidden
    ...
    $stmt = $pdo->prepare('SELECT u.id, u.first_name, u.last_name, u.email, u.role AS usertype, COALESCE(m.name, "") AS municipality, u.created_at FROM users u LEFT JOIN municipalities m ON u.municipality_id = m.id WHERE u.id = ?');
    $stmt->execute([$id]);
    $r = $stmt->fetch();
    if (!$r) {
    ...
    'profile_picture' => $r['profile_picture'] ? $r['profile_picture'] : 'assets/images/default-avatar.jpg',
        ...
        'contact_number' => $r['contact_number'] ?? $contact_number,
        'usertype' => $r['usertype'],
        'status' => '',
        'email' => $r['email'],
