...
if ($isFormData) {
    ...
    $usertype = trim($_POST['usertype'] ?? $_POST['role'] ?? 'user');
    $status = isset($_POST['status']) ? trim($_POST['status']) : null;
    $municipalityName = trim($_POST['municipality'] ?? '');
...
if (isset($_FILES['profile_picture']) && $_FILES['profile_picture']['error'] === UPLOAD_ERR_OK) {
    ...
    if (in_array($_FILES['profile_picture']['type'], $allowedTypes) && $_FILES['profile_picture']['size'] <= 2 * 1024 * 1024) { // 2MB limit
...
try {
    ...
    $roleChanging = $usertype !== $currentUserRole;
    if (($roleChanging && $currentRole !== 'administrator') || ($status !== null && $currentRole !== 'administrator')) {
    ...
    $updateFields = ['first_name = ?', 'last_name = ?', 'email = ?', 'role = ?', 'municipality_id = ?', 'address = ?', 'contact_number = ?'];
    $params = [$first_name, $last_name, $email, $usertype, $municipality_id, $address, $contact_number];
    if ($status !== null) {
        $updateFields = array_merge(array_slice($updateFields, 0, 4), ['status = ?'], array_slice($updateFields, 4));
        $params = array_merge(array_slice($params, 0, 4), [$status], array_slice($params, 4));
    }
