...
class ChatServer implements MessageComponentInterface {
    ...
    public function onClose(ConnectionInterface $conn) {
        $user_id = $this->users[spl_object_hash($conn)] ?? null;
        if ($user_id && isset($this->userConnections[$user_id])) {
            // Remove this connection from user's connections
            $this->userConnections[$user_id] = array_filter($this->userConnections[$user_id], function($c) use ($conn) {
                return $c !== $conn;
            });
